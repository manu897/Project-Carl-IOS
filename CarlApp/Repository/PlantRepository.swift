...
protocol PlantRepository: Sendable {
    ...
    func add(mac: String, keyHex: String, name: String) async throws -> Plant
...
final class HubRepository: PlantRepository, @unchecked Sendable {
    ...
    func add(mac: String, keyHex: String, name: String) async throws -> Plant {
        try await client.createNode(NodeCreate(mac: mac, keyHex: keyHex, name: name, calibration: nil))
    }

    func rename(id: String, to name: String) async throws -> Plant {
        try await client.updateNode(id: id, with: NodeUpdate(name: name, calibration: nil))
    }

    func updateCalibration(id: String, _ calibration: Calibration) async throws -> Plant {
        try await client.updateNode(id: id, with: NodeUpdate(name: nil, calibration: calibration))
    }
